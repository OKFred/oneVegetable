...
- 真实 mutation 继续逐方法门禁。`saveProductDraft` 已在 2026-08-21 通过真实账号完成新增、`schema.render.draft` 回读和国际站官方编辑页同 ID 保存验证，因此只在 `pnpm dev:api:real` 的本地 Node 环境默认开放。OpenAPI 没有覆盖保存既有平台草稿的接口；创建后的本地修改继续自动保存，并提供“在国际站继续编辑”入口。正式发布、正式商品更新、staging、production 和扩展商品写入继续关闭。
