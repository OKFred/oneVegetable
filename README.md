...
- 真实 mutation 继续逐方法门禁。`saveProductDraft` 已在 2026-08-21 通过真实账号完成新增、`schema.render.draft` 回读和国际站官方编辑页同 ID 保存验证；`updateProduct` 已在 2026-08-22 对正式商品完成单字段增量更新及原值恢复；`publishProduct` 已在 2026-08-28 完成单品发布以及 3 条严格串行批量发布、列表回读和防重复重跑验证。2026-09-01 又通过本地构建的 MV3 service worker 分别创建并回读草稿 `1601938471572` 和发布商品 `1601938537310`，因此插件现已开放 `saveProductDraft`、`publishProduct` 和批量发品。所有扩展商品写入都会在出网前保存最小化本地任务；结果不确定时阻止相同内容重复提交，新增商品不会自动删除或下架。`updateProduct` 仍只在本地 Node BFF 开放；商品分组新增、staging、production 和其他扩展商品写入继续按各自验证状态与门禁执行。
