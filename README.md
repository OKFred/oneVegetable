...
- 真实 mutation 继续逐方法门禁。`saveProductDraft` 已在 2026-08-21 通过真实账号完成新增、`schema.render.draft` 回读和国际站官方编辑页同 ID 保存验证；`updateProduct` 已在 2026-08-22 对正式商品完成单字段增量更新及原值恢复；`publishProduct` 已在 2026-08-28 完成真实发布和列表回读。三项能力只在 `pnpm dev:api:real` 的本地 Node 环境按独立 flag 开放。批量发品只是对这些已受保护的单品 operation 做串行编排，本轮未执行多商品真实发布；批量上下架、商品分组新增、staging、production 和扩展商品写入仍按各自验证状态与门禁执行。
