...
- 真实 mutation 继续逐方法门禁。`saveProductDraft` 已在 2026-08-21 通过真实账号完成新增、`schema.render.draft` 回读和国际站官方编辑页同 ID 保存验证；`updateProduct` 已在 2026-08-22 对正式商品完成单字段增量更新及原值恢复；`publishProduct` 已在 2026-08-28 完成单品发布以及 3 条严格串行批量发布、列表回读和防重复重跑验证。三项能力只在 `pnpm dev:api:real` 的本地 Node 环境按独立 flag 开放。批量发品继续复用受保护的单品 operation，不绕过权限或契约门禁。2.1.0 之后的插件补丁版本允许商品批量上下架：service worker 会在出网前保存最小化本地任务、核对实时商品状态，并在 Alibaba 明确受理后持续回读；超时任务会阻止重复操作并提供恢复原状态入口。商品分组新增、staging、production 和其他扩展商品写入仍按各自验证状态与门禁执行。
