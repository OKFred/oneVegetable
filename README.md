...
- 真实 mutation 继续逐方法门禁。`saveProductDraft` 已在 2026-08-21 通过真实账号完成新增、`schema.render.draft` 回读和国际站官方编辑页同 ID 保存验证；`publishProduct` 已在 2026-08-28 完成单品发布以及 3 条严格串行批量发布、列表回读和防重复重跑验证。2026-09-01 又通过本地构建的 MV3 service worker 分别创建并回读草稿 `1601938471572` 和发布商品 `1601938537310`。2026-09-06，插件对该测试商品完成单字段 `updateProduct`、异步审核、Schema 指纹回读及原值恢复，并新增、回读后保留了商品分组 `971947645`；账号一级分组已满，因此测试分组创建在现有父级下。插件现已开放 `saveProductDraft`、`publishProduct`、`updateProduct`、`updateProductDisplay`、`createProductGroup` 和批量发品。所有扩展商品写入都会在出网前保存最小化本地任务；结果不确定时阻止重复提交，新增商品不会自动删除或下架。staging、production 和其他扩展商品写入继续按各自验证状态与门禁执行。
