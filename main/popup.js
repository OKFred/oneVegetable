...
(async function preCheck() {
	...
	let setArr=[needQRcode, userLocale==='zh_HK', needCloudService];
	...
	if (maStatus) {
		...
		if (userLocale === 'zh_HK') BG.taskMa.changeLanguage(); //默认值是英语，如果设置了就切换
	} else {
...
Check5.addEventListener('click', async()=>{
	if (Check5.checked){
		BG.users.default.maData.language='zh_HK';
	...
	return await BG.write({ "userLocale": Check5.checked ? 'zh_HK' : 'en_US' }, "sync");
}); //语言切换
