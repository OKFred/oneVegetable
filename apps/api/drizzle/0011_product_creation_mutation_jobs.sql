DROP INDEX IF EXISTS product_mutation_jobs_request_target_unique;
...
ALTER TABLE product_mutation_jobs RENAME TO product_mutation_jobs_v10;
...
CREATE TABLE product_mutation_jobs (
  ...
  operation TEXT NOT NULL CHECK (
    operation IN ('publishProduct', 'saveProductDraft', 'updateProduct', 'updateProductDisplay')
  ),
  ...
  CHECK (
    (
      operation IN ('publishProduct', 'saveProductDraft', 'updateProduct')
...
SELECT
  ...
  submitted_time_utc, last_checked_time_utc, completed_time_utc,
  create_time_utc, update_time_utc, creator_id, updater_id, revision, remark
FROM product_mutation_jobs_v10;

DROP TABLE product_mutation_jobs_v10;
...
CREATE UNIQUE INDEX product_mutation_jobs_open_creation_fingerprint_unique
  ON product_mutation_jobs (operation, payload_fingerprint)
  WHERE operation IN ('publishProduct', 'saveProductDraft')
