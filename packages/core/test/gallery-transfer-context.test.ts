...
import { galleryGatewayId, galleryStorageId, assertGalleryContextId, requireGalleryContext } from '../src/gallery-transfer-context';
...
describe('opaque gallery execution context', () => {
  it('keeps normal bundle token refresh stable but rejects a new account bundle', async () => {
    const credentials = { endpoint: 'https://eco.taobao.com/router/rest', appKey: 'test', appSecret: 'secret', accessToken: 'first', signMethod: 'hmac' as const, galleryAccountGeneration: 'bundle-1' };
  ...
  it('pins endpoint, bucket, prefix and credentials without returning them', async () => {
    ...
    expect(() => assertGalleryContextId(id, 'changed')).toThrow('GALLERY_CONTEXT_CHANGED');
    expect(() => assertGalleryContextId(undefined, id)).not.toThrow();
  });
  it('rejects extra fields and invalid context input', () => {
    expect(() => requireGalleryContext({ identity: 'a', gateway: 'b', storage: null, token: 'secret' })).toThrow();
