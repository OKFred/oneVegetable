  ...
  releases: [
    {
      ...
      title: {
        'zh-CN': '统一列表、素材上传与编辑保护',
        'en-US': 'Consistent lists, media uploads and editing safeguards'
      },
      summary: {
        'zh-CN':
          '统一列表筛选、列设置和操作，新增库存工作区、全屏素材预览及视频上传。编辑不再自动恢复，离开前提醒未保存修改。',
        'en-US':
          'Unifies list filters, columns and actions, adds an inventory workspace, fullscreen media preview and video uploads. Unsaved edits prompt before leaving instead of being restored automatically.'
      },
      source: 'release',
      githubUrl: 'https://github.com/OKFred/oneVegetable/releases/tag/v2.12.0',
      compareUrl: 'https://github.com/OKFred/oneVegetable/compare/v2.11.0...v2.12.0',
      changes: [
        {
          type: 'improvement',
          title: {
            'zh-CN': '统一列表与库存入口',
            'en-US': 'Consistent lists and inventory access'
          },
          description: {
            'zh-CN':
              '列显隐及排序保存在本机，统一筛选弹窗、选择和操作菜单。库存可独立查询，产品分按可见区域加载。',
            'en-US':
              'Column visibility and order are saved locally, with consistent filters, selection and action menus. Inventory has its own workspace; product scores load when visible.'
          }
        },
        {
          type: 'feature',
          title: {
            'zh-CN': '素材预览与视频上传',
            'en-US': 'Media preview and video uploads'
          },
          description: {
            'zh-CN':
              '图片与视频共用全屏预览及信息面板。Node 和插件支持 HTTPS 地址或最大 50 MiB MP4 上传、S3 分片及回读核对；Worker 真实上传仍关闭，结果不明不自动重发。',
            'en-US':
              'Images and videos share fullscreen previews and metadata. Node and extension support HTTPS URLs or MP4 files up to 50 MiB, S3 multipart upload and readback. Worker real uploads remain disabled; uncertain writes are not resent.'
          }
        },
        {
          type: 'fix',
          title: {
            'zh-CN': '编辑、解锁与反馈',
            'en-US': 'Editing, unlocking and feedback'
          },
          description: {
            'zh-CN':
              '未保存修改离开前确认；批量提交错误改用弹窗。插件锁定时提供可关闭的解锁提示，统一按钮与反馈，不增加永久权限。',
            'en-US':
              'Confirms before discarding edits and shows batch submission errors in a dialog. Locked extensions offer a dismissible unlock prompt. Buttons and feedback are consistent, without new permanent permissions.'
          }
        }
      ]
    },
      ...
      changes: [
        {
          ...
          title: {
            'zh-CN': '商品与 SKU 库存查询',
            'en-US': 'Product and SKU inventory queries'
          },
          description: {
            'zh-CN': '按需切换接口来源，区分空记录、真实零值及数据待核对，不推算总库存。',
            'en-US':
              'Choose the API source explicitly. Empty records, true zero and contract issues are distinct; no total stock is inferred.'
          }
        },
        {
          type: 'feature',
          title: {
            'zh-CN': '批量查询与共享缓存',
            'en-US': 'Batch queries and shared cache'
          },
          description: {
    ...
    },
    {
      ...
      title: {
        'zh-CN': '橱窗调整与插件稳定性（候选）',
        'en-US': 'Showcase editing and extension reliability (candidate)'
      },
      summary: {
        'zh-CN':
          '发布候选，尚未上架。新增橱窗排序、替换和类目发品类型引导，修复插件能力校验与后台重启恢复。',
        'en-US':
          'Unpublished candidate. Reorder and replace showcase products, choose category-specific posting types, and reliably validate capabilities after worker restarts.'
      },
      source: 'release',
      githubUrl:
        'https://github.com/OKFred/oneVegetable/blob/staging/docs/release-2.7.0-readiness.md',
      compareUrl: null,
      changes: [
        {
          type: 'feature',
          ...
          description: {
            'zh-CN':
              '确认后调整顺序或替换商品，并回读核对。结果不明时不自动重试。插件和本地 Node 支持，云端写入策略不变。',
            'en-US':
              'Confirm changes, then verify by read-back. Uncertain writes are never automatically retried. Available in the extension and local Node; cloud policy is unchanged.'
          }
        },
        {
          type: 'feature',
          title: {
            'zh-CN': '类目发品类型引导',
            'en-US': 'Category-specific posting types'
          },
          description: {
            'zh-CN':
              '新建商品时查询类目支持的直接下单或询盘定制类型，再加载对应表单；不可用时明确提示。',
            'en-US':
              'Check whether a category supports direct orders or inquiry/custom products before loading its form, with clear unavailable-state guidance.'
          }
        },
        {
          type: 'fix',
          title: {
            'zh-CN': '插件能力校验',
            'en-US': 'Extension capability validation'
          },
          description: {
            'zh-CN':
              '七个领域使用 MV3 静态校验模块；后台重启后仍可读取能力和校验请求，缺失校验器时安全拒绝。',
            'en-US':
              'Seven domains use static MV3 validators. Catalog reads and request validation survive worker restarts; missing validators fail safely.'
          }
        }
      ]
    },
    {
      ...
      title: {
        'zh-CN': '图库任务、商品橱窗与凭据管理',
        'en-US': 'Gallery tasks, showcases and credentials'
      },
      summary: {
        'zh-CN':
          'GitHub 版本已发布，商店已提交审核。新增图库任务恢复、商品橱窗、列表自定义列与 Node 后端凭据配置。',
        'en-US':
          'Released on GitHub and submitted for Store review. Gallery recovery, product showcases, configurable columns and Node credential settings.'
      },
      source: 'release',
      githubUrl:
        'https://github.com/OKFred/oneVegetable/blob/master/docs/release-2.6.0-readiness.md',
      compareUrl: null,
      changes: [
        {
          type: 'feature',
          title: {
            'zh-CN': '跨页面传输',
            'en-US': 'Cross-page transfers'
          },
          description: {
            'zh-CN':
              '关闭弹窗不停止；刷新后确认恢复，成功项不重传，未知结果先核对。Web 与插件均支持。',
            'en-US':
              'Continue across dialogs; confirm after reload. Keep successful receipts and verify uncertain writes. Available in Web and extension.'
          }
        },
        {
          type: 'feature',
          title: {
            'zh-CN': '商品橱窗管理',
            'en-US': 'Product showcases'
          },
          description: {
            'zh-CN':
              '查看额度与商品，确认后批量加入或移出并回读核对。插件和本地 Node 支持，云端写入策略不变。',
            'en-US':
              'Inspect quota and products, confirm additions or removals, then verify by read-back. Available in the extension and local Node; cloud write policy is unchanged.'
          }
        },
        {
          type: 'feature',
          title: {
            'zh-CN': 'Node 凭据配置',
            'en-US': 'Node credential settings'
          },
          description: {
            'zh-CN':
              '管理员可在设置页导入或填写凭据、测试连接、更换和清除。加密保存即时生效，清除后不回退旧配置。',
            'en-US':
              'Administrators can import or enter credentials, test, replace and clear them in Settings. Encrypted changes apply immediately; clearing never revives old settings.'
          }
        },
        {
          type: 'improvement',
          title: {
            'zh-CN': '列表与查询体验',
            'en-US': 'Lists and queries'
          },
          description: {
            'zh-CN':
              '商品、图库和订单支持自定义列；相关列按需加载扩展数据。商品链接使用独立按钮，标题、分组、状态和产品分列更紧凑。',
            'en-US':
              'Customize product, gallery and order columns; relevant columns load extra data on demand. Product links use dedicated buttons, with compact title, group, status and score columns.'
          }
        },
        {
          type: 'improvement',
          title: {
            'zh-CN': '能力目录与安全边界',
            'en-US': 'Capabilities and safeguards'
          },
          description: {
            'zh-CN':
              '补充国家、发品类型、库存和橱窗等类型化能力。无权限和空结果明确区分，不以模拟数据替代真实错误。',
            'en-US':
              'Adds typed country, product-type, inventory and showcase capabilities. Permission denials and empty results stay distinct; real errors never fall back to sample data.'
          }
        }
      ]
    },
