export const capabilities = {
  ...
  matrix: {
    ...
    account: {
      passed: ['账号通过', '历史 smoke 返回有效数据。'],
      noData: ['合法空结果', '历史 smoke 成功，但账号当时没有数据。'],
      denied: ['账号无权限', '历史 smoke 被平台权限拒绝。'],
      drift: ['契约漂移', '历史 smoke 响应不符合当前契约。'],
      provider: ['上游错误', '历史 smoke 遇到平台或网络错误。'],
      prerequisite: ['缺前置数据', '历史 smoke 缺少可用于调用的真实前置数据。'],
