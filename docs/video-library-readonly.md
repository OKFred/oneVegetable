...
1. 文档示例分别要求查询 `msg_code=200`、关联 `msg_code=0`，真实成功响应未返回此字段。适配器只在成功数据结构完整时接受，保留 `result/msg_code:not-returned` 诊断记录，但界面不再为此展示图标、内容或 Toast；若返回成功码则严格按方法校验，显式失败绝不接受。其他未确认的差异与实际错误仍正常展示。
