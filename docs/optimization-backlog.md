...
- 功能分支从最新 `origin/staging` 创建；先确认 staging 包含 master 的正式发布收尾。
- 每完成一个独立待办即提交一次，避免把多个风险点压进同一个提交。
- 后续开始新功能前，确保 `staging` 已包含 `master` 的发布与隐私协议提交。

## 2026-09-21：2.10 开发方向

- v2.9.0 已完成 GitHub Release；网站部署与商店提交单独确认，不能由 GitHub 发布状态推断。
- 商品视频选用与关联：[实现范围和真实验收门槛](product-video-association.md)。先提供现存商品选片、预览、二次确认、持久回执和只读核对；真实写入保持关闭。
- 继续保持最终 Store ZIP **1,500,000 字节**、解包 **4,100,000 字节**、background **100,000 字节**、共享 i18n **310,000 字节**门槛；本页历史段落的旧预算不是当前门槛。
- CI 失败时保留 Mock/Replay Playwright 诊断，不上传真实账号、授权包、数据库或浏览器 Profile。
