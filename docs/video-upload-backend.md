...
回读 `confirmed` 的任务可点击“用于商品”，在重新核对账号和存储上下文后进入独立的商品选择弹窗。弹窗重新查询视频 ID 和加密 ID，再复用商品视频关联确认及回执流程；不会因上传成功自动修改商品。账号或配置改变时旧任务不可直接选用。真实关联尚未验收，按钮仍受原有禁用策略约束；详见 [商品视频选用与关联](product-video-association.md)。
