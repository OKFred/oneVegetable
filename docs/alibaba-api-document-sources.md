...
2026-09-10 补充：对淘宝官方详情明确标为 ICBU、免费且不需用户授权，但国际站旧目录未收录的方法，允许保留独立 TOP 补充审计并进行用户明确授权的少量验证。不得把补充条目当作国际站目录重审结果；应用 API 权限仍由平台判断。文本翻译的用户提供文档、差异及权限拒绝证据见 `icbu-text-translation.md`，未标记 account-verified。
