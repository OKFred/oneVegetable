...
普通 `table-layout: auto` 在只剩必选列时仍可能忽略 max-width，把商品列撑到约 56%。补充采用受限表格固定布局；没有其他弹性业务列时，在固定操作列之前放置不进入列菜单、不可交互且对辅助技术隐藏的布局余量列，避免拉伸商品或固定列。其他未声明宽度上限的表格维持原布局。

补充的列宽边界、列设置、暗色模式及无障碍 6 项浏览器测试通过，表格和商品工具栏 14 项单测通过。Web/MV3 重建与商店检查通过，最终扩展总量为 4,082,848 字节，双语块仍为 309,933 字节。最终全量浏览器回归日志为 `ui-fixes-final-e2e-20260911.log`。
