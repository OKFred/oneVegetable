...
- `ONE_VEGETABLE_MUTATION_FLAGS` 默认空值。`pnpm dev:api:real` 仅在本地 Node 子进程中默认注入
  `operation:publishProduct,operation:saveProductDraft,operation:updateProduct,operation:updateProductDisplay,operation:createProductGroup,operation:operatePhotoGroup,operation:uploadPhoto,operation:transferPhotoFromUrl`。除商品分组新增外，这些操作已经完成真实账号 Smoke；商品分组新增依据官方契约接入，本地界面会在每次提交前明确提示该操作目前无法通过 OpenAPI 改名或删除。商品上下架通过持久任务提交并用商品列表回读最终状态，上架可能进入平台异步审核，审核期间禁止重复提交。新增商品和正式商品增量更新同样可能触发平台审核，审核期间读取 Schema 可能返回 `PUB_BIZCHECK_PRODUCT_IN_AUDITING`，客户端应保留本地草稿并稍后刷新。
...
真实正式发布通过 `ONE_VEGETABLE_REAL_PRODUCT_PUBLISH_SMOKE=1 pnpm smoke:product:publish:real` 显式执行。脚本先复用当前账号可渲染且满足名称、主图最低发布条件的商品作为测试模板，只改标题标记；其他 Schema 预检警告交由 Alibaba 最终校验。平台返回商品 ID 后立即写入忽略目录中的脱敏报告，后续只能恢复回读，不能重复创建。默认保留 Alibaba 当前发布状态，不主动下架；只有额外设置 `ONE_VEGETABLE_REAL_PRODUCT_PUBLISH_CLEANUP=1` 时才会在回读到 `online` 后下架该次 Smoke 商品。2026-08-28 的验收商品 `1601935651469` 已由列表回读确认标题一致。
