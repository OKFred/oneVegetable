...
- [ ] 在未预授权的 Chrome 中验证 2.5.0 S3 配置首次保存：手工允许精确 Endpoint 域名，确认保存成功；隔离预授权副本的真实读写通过不替代此验收；
- [ ] 上传 2.5.0 图库 S3 导入导出包，核对中英文说明及可选 S3 主机权限披露后再提交审核；本地打包、主线合并不代表已上传、提交审核或上架；
