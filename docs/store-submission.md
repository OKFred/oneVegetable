...
检查日期：2026-09-22。当前发布候选为 **2.11.0**；商店上传、提交审核和实际上架是三个独立状态，不能根据本地版本或 GitHub Release 推断。本次尚未查询商店后台状态，不覆盖或复用旧版本包。

本版新增 35 项类型化能力及平台同步延迟提示，无新增 Manifest 权限、数据库 migration 或默认开放的写操作。提交前使用 [2.11.0 发布交接](release-2.11.0-readiness.md) 核对包、验收边界和中英文摘要；下方历史交接仅为历史记录。
...
- 平台草稿、正式发品、商品更新/上下架、商品分组创建、橱窗加入/移出/排序/替换、图库分组管理、图片上传和外部图片转存只在用户主动操作后开放；能力验证范围和真实验收记录见各候选报告，其他 mutation 在出网前关闭；
