...
状态：2.5.0 已由项目所有者确认上架；2.6.0 仍是候选，不是已发布版本。功能及发布准备统一从 `codex/release-2.6.0-readiness` 推进至 `staging`、`master`；运行内容基线为 `013db1c`，`b4162b7` 仅修正登录过期回归测试时序，后续交接记录不改变运行包。本轮没有创建发布 Tag、GitHub Release、部署 Worker 或上传、提交 Chrome Web Store。
...
1. 发布准备分支按用户要求快进至 `staging`、`master`；合并不等于发布，上传前核对本节顶部的最新候选 SHA-256。
