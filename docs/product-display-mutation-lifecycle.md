...
2.2.0 起，插件允许本机管理员执行已完成真实账号验收的商品上下架。插件不直接复用 BFF 数据库，而是在 `chrome.storage.local` 中保存同构的最小任务快照：商品明文 ID、混淆 ID、原状态、目标状态、requestId、traceId、状态和脱敏错误。任务不包含 App Secret、Access Token、完整请求或完整 Alibaba 响应。
...
2.2.0 同时开放已经完成插件真实验收的正式发品和平台草稿；两者使用独立的持久任务、指纹去重和平台回读闭环。

2026-09-06，插件对测试商品 `1601938537310` 完成一次单字段 Schema 增量更新、平台异步审核、字段指纹回读以及原标题恢复。`updateProduct` 因而在扩展中开放，并复用同一持久任务仓库：出网前只保存商品、类目、语言、请求指纹和各字段指纹，不保存完整 XML；平台受理后进入 `auditing`，通过 `schema.render` 回读匹配后才进入 `verified`。网络结果不确定、字段缺失或值不一致时阻止重复提交并要求人工核对。

MV3 service worker 不包含 DOM/XML 实现。受信的 `options.html` 在发送更新前解析增量 XML 并生成字段指纹，后台重新计算整段 XML 的 SHA-256 后才允许出网；回读时同一受信页面解析 `schema.render`，后台只接受与任务字段集合一致的比较结果。Alibaba 内容脚本、Popup 和其他扩展页面都不能提交更新或完成回读。这样既保留持久任务边界，也避免把 DOM 库打入后台脚本。其他未经插件路径验收的真实写操作仍保持关闭。
