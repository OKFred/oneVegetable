...
3. 在 Windows 上执行完整发布前检查：
   ...
   ```powershell
   pnpm check:release-version v2.1.0
   pnpm check
   pnpm test:e2e
...
4. 确认待发布提交已经推送且属于远端 `master`，再创建 annotated tag：
   ...
   ```powershell
   git tag -a v2.1.0 -m "oneVegetable v2.1.0"
...
- 需要较长冻结期时，创建 `release/2.1.0`，只接受版本、文档和阻断发布的问题修复；最终仍合入 `master` 后再打 Tag。
- 紧急修复从最近正式 Tag 创建 `hotfix/2.1.0`，完成后同时合回 `master` 和 `staging`，并发布新的 patch 版本。不要修改原 Tag 或覆盖原 Release。
