...
平台草稿创建只要求类目、语言、请求契约和 XML 结构安全；Schema 内容错误继续提示，但不在前端提前阻止 `schema.add.draft`。平台草稿成功后，后续更改仅保存到本地，避免重复创建；重新进入时使用 `schema.render.draft` 读取平台基线。直接发布和正式更新仍要求 Schema 硬错误清零。
